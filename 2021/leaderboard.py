...
def leaderboard(id):
    if not GET_API:
        A = ""
        with open(f"leaderboard_{YEAR}_{id}.json", "r", encoding='utf-8') as f:
            for l in f:
                A += l
        a = eval(A.replace('null','None'))
    else:
        url=f'https://adventofcode.com/{YEAR}/leaderboard/private/view/{id}.json'
        ...
        #r.encoding='utf-8'
        t=r.text
        with open(f"leaderboard_{YEAR}_{id}.json", "w", encoding='utf-8') as f:
            f.write(t)
        a=eval(t.replace('null','None'))


    days = [datetime(2021, 12, i, 6) for i in range(1, 26)]

    with open(f"leaderboard_{YEAR}_{id}.txt", "w", encoding='utf-8') as out:
        out.write(a["event"] + "\n")
        mem = a["members"]
        best = {}
